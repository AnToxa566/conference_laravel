						...
						@if ($conference->users->find(auth()->user()->id))
							<div class="col-2">
								<form action="{{ route('users.cancel', auth()->user()->id) }}" method="POST">
									...
									<input type="submit" value="Сancel participation" class="btn btn-lite p-2 rounded shadow-sm w-100" style="font-size: 14px;">
								</form>
							</div>

							<div class="col-2 d-flex justify-content-evenly"> 
								<a href="https://www.facebook.com/sharer/sharer.php?u={{urlencode(route('conferences.show', $conference->id))}}" title="Share the conference in Facebook">
								...
								<a href="http://twitter.com/share?text={{__('Check out this Meetup with SoCal AngularJS!')}}&url={{urlencode(route('conferences.show', $conference->id))}}" title="Share the conference in Twitter">
